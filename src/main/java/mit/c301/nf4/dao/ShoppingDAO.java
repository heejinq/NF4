...
public interface ShoppingDAO {
	...
	SigninVO signin(String id); 							// 회원정보

	Order_infoVO orderinfo(int order_no);					// 마이페이지 - 주문내역 - 주문상세정보(주문자,배송정보) page11
	
	List<OrderdetailsVO> orderdetails(int order_no);	 	// 마이페이지 - 주문내역 - 주문상세정보(주문상품정보) page11
	...
	// 희진쌤
	ProductVO recommendView(@Param("p_type")int p_type, @Param("gender")int gender);	
	...
	int questionWrite(@Param("all_questinoVO")All_questionVO all_questinoVO, @Param("id")String id);	//문의글등록

	int valueNumGT(ProductVO productvo);

	List<ProductVO> pagingProductList(Limit_ProductlistVO vo);
}
